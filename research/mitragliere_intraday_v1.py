# mitragliere_intraday_v1.py — Intraday Research Engine (15m/5m)
# ---------------------------------------------------------------
# Obiettivo: aggiungere al tuo arsenale un set di strategie Intraday ("Mitragliere")
# senza toccare il sistema "Cecchino". Questo modulo esegue backtest su:
# 1) Mean Reversion con Bande di Bollinger + RSI
# 2) Breakout da compressione (Donchian/Keltner-like) con filtro volume
#
# Output:
# - Classifica strategie per asset (metriche estese)
# - Salvataggio automatico delle migliori logiche in hall_of_fame_intraday.json
#
# Dipendenze: pandas, pandas_ta, (tuo) data_sources.binance_client
# Directory consigliata: research/mitragliere_intraday_v1.py
#
# NOTE:
# - È un motore di ricerca separato: non interferisce con l'ETL esistente.
# - Parametri e logiche sono modulari e semplici da espandere.
# - Pensato per timeframe 15m di default, con opzione 5m.
# ---------------------------------------------------------------
...
def add_intraday_indicators(df: pd.DataFrame, params: dict) -> pd.DataFrame:
    ...
    df['BBL'] = bb[f'BBL_{bb_len}_{bb_mult}']
    df['BBM'] = bb[f'BBM_{bb_len}_{bb_mult}']
    df['BBU'] = bb[f'BBU_{bb_len}_{bb_mult}']
...
def signal_mean_reversion(df: pd.DataFrame, i: int, params: dict):
    ...
    if (c_prev['low'] <= c_prev['BBL']) and (c_prev['RSI'] <= rsi_buy):
        entry = c_now['close']
        sl = min(c_prev['low'], c_prev['BBL']) - 0.1 * df.iloc[i-1][f'ATR_{atr_len}']
    ...
    if (c_prev['high'] >= c_prev['BBU']) and (c_prev['RSI'] >= rsi_sell):
        entry = c_now['close']
        sl = max(c_prev['high'], c_prev['BBU']) + 0.1 * df.iloc[i-1][f'ATR_{atr_len}']
...
def signal_breakout(df: pd.DataFrame, i: int, params: dict):
    ...
    atr = prev[f'ATR_{atr_len}'] if not pd.isna(prev[f'ATR_{atr_len}']) else 0
...
def backtest_intraday(df: pd.DataFrame, params: dict, logic_name: str, session_hours=None):
    ...
    for i in range(start, len(df)):
        if active:
            ...
            if res:
                active['exit_time'] = df.iloc[i]['timestamp']
                active['result'] = res
                trades.append(active)
                active = None

        if not active:
            if logic_name == 'MR_BB_RSI':
                sig = signal_mean_reversion(df, i, params)
            elif logic_name == 'BRK_COMP_VOL':
                sig = signal_breakout(df, i, params)
            else:
                sig = None
            if sig:
                sig['entry_time'] = df.iloc[i]['timestamp']
                active = sig
...
def grid_search_intraday(df, logic_name, param_grid, session_hours=None):
    ...
    for params in param_grid:
        ...
        row = {**params, **res}
        results.append(row)
    results = sorted(results, key=lambda x: (x['profit_factor'], x['win_rate'], x['avg_r_per_trade']), reverse=True)
    return results
...
if __name__ == '__main__':
    ...
    MR_GRID = [
        {"bb_len": 20, "bb_mult": 2.0, "rsi_len": 14, "rsi_buy": 30, "rsi_sell": 70, "atr_len": 14, "use_rr_mr": True, "rr_mr": 1.2},
    ...
    BRK_GRID = [
        {"dc_len": 20, "atr_len": 14, "min_compression_atr": 0.6, "vol_sma_len": 20, "volume_multiplier": 1.2, "rr_brk": 1.6},
    ...
    hof = {}
    try:
        with open('hall_of_fame_intraday.json', 'r') as f:
            hof = json.load(f)
            logging.info("Caricato hall_of_fame_intraday.json esistente (verrà aggiornato).")
    except FileNotFoundError:
        logging.info("Creerò un nuovo hall_of_fame_intraday.json.")
    ...
    for symbol in ASSETS:
        ...
        try:
            ...
            if not klines:
                logging.warning(f"Nessun dato per {symbol}. Salto.")
                continue
            df = prepare_dataframe(klines)
            logging.info(f"Dati caricati: {len(df)} barre")
        except Exception as e:
            logging.error(f"Errore dati {symbol}: {e}")
            continue

        mr_results = grid_search_intraday(df.copy(), 'MR_BB_RSI', MR_GRID, session_hours=SESSION_HOURS)
        best_mr = mr_results[0] if mr_results else None

        brk_results = grid_search_intraday(df.copy(), 'BRK_COMP_VOL', BRK_GRID, session_hours=SESSION_HOURS)
        best_brk = brk_results[0] if brk_results else None

        candidates = [x for x in [best_mr, best_brk] if x]
        if candidates:
            best = sorted(candidates, key=lambda x: (x['profit_factor'], x['win_rate'], x['avg_r_per_trade']), reverse=True)[0]
            hof[symbol] = best
            logging.info(f"🥇 Migliore su {symbol}: {best['name']} PF {best['profit_factor']} WR {best['win_rate']}% R {best['avg_r_per_trade']}")
            all_rows.append({**best, "asset": symbol})
        else:
            logging.info(f"Nessuna strategia valida su {symbol} in questa run.")

    if all_rows:
        dfres = pd.DataFrame(sorted(all_rows, key=lambda x: (x['profit_factor'], x['win_rate'], x['avg_r_per_trade']), reverse=True))
        print("\n" + "="*80)
        print("  CLASSIFICA INTRADAY (Mitragliere)")
        print("="*80)
        print(dfres.to_string(index=False))
        print("="*80)

    with open('hall_of_fame_intraday.json', 'w') as f:
        json.dump(hof, f, indent=4, sort_keys=True)
...
# ISTRUZIONI:
# - Esegui:  python research/mitragliere_intraday_v1.py
# - Cambia TF a '5m' per maggiore frequenza (riduci YEARS a 0.5)
# - Integra i best nel bot intraday live con ordini limit/stop e trailing ATR.
